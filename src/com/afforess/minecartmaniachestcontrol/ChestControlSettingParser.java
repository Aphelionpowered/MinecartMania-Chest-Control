...
public class ChestControlSettingParser implements SettingParser {
            ...
            for (int idx = 0; idx < list.getLength(); idx++) {
                ...
                if (elementChild.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if ((elementChildValue != null) && (elementChildValue != "")) {
                        ...
                        } else if ((elementChildName == "ChestDispenserSpawnDelay") || (elementChildName == "ItemCollectionRange")) {
    ...
    private int getDefaultConfigurationIntegerValue(final String ConfigName) {
        if (ConfigName == "ChestDispenserSpawnDelay")
            return (1000);
        if (ConfigName == "ItemCollectionRange")
            return (1);
        return 0;
    }
    
    private double getDefaultConfigurationDoubleValue(final String ConfigName) {
        if (ConfigName == "SpawnAtSpeed")
            return (0.0);
        return 0;
    }
    ...
    private void debugShowConfigs() {
        ...
        for (final Enumeration<String> ConfigKeys = MinecartManiaWorld.getConfiguration().keys(); ConfigKeys.hasMoreElements();) {
            ...
            if ((temp == "SpawnAtSpeed") || (temp == "ChestDispenserSpawnDelay") || (temp == "ItemCollectionRange")) {
