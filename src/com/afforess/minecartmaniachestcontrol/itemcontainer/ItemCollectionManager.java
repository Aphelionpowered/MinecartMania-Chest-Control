...
public class ItemCollectionManager {
    ...
    public static ArrayList<ItemContainer> getItemContainers(Location location,
            CompassDirection direction, boolean collection) {
    ...
    public static ArrayList<ItemContainer> getTrashItemContainers(
            Location location, CompassDirection direction) {
        ArrayList<ItemContainer> containers = new ArrayList<ItemContainer>();
        ArrayList<String> lines = getItemLines(((Sign) location.getBlock().getState()));
        for (String text : lines) {
            if (!text.isEmpty() && !isFurnaceFuelLine(text) && !isFurnaceSmeltLine(text) && !isBrewingStandBottomLine(text) && !isBrewingStandTopLine(text)) {
    ...
    public static ArrayList<ItemContainer> getFurnaceContainers(
            Location location, CompassDirection direction) {
        ArrayList<ItemContainer> containers = new ArrayList<ItemContainer>();
        HashSet<Block> blocks = BlockUtils.getAdjacentBlocks(location, 1);
        for (Block block : blocks) {
    ...
    public static ArrayList<ItemContainer> getBrewingStandContainers(
            Location location, CompassDirection direction) {
        ArrayList<ItemContainer> containers = new ArrayList<ItemContainer>();
        HashSet<Block> blocks = BlockUtils.getAdjacentBlocks(location, 1);
        for (Block block : blocks) {
    ...
    public static void createItemContainers(MinecartManiaStorageCart minecart,
            HashSet<ComparableLocation> available) {
        ArrayList<ItemContainer> containers = new ArrayList<ItemContainer>();
    ...
    public static void updateContainerDirections(
