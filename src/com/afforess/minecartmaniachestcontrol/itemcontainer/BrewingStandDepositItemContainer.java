                        ...
                        if (brewingStand.canRemoveItem(item.getId(), toRemove, data)) {
                            if (deposit.canAddItem(item.toItemStack())) {
                                if (deposit.addItem(item.toItemStack())) {
                                    ...
                                    MinecartManiaLogger.getInstance().info("[BrewingStand:Deposit] Can't add " + item.toItemStack().toString());
                                    return;
                                }
                            } else {
                                MinecartManiaLogger.getInstance().info("[BrewingStand:Deposit] Can't deposit " + item.toItemStack().toString());
                            }
                        } else {
                            MinecartManiaLogger.getInstance().info("[BrewingStand:Deposit] Can't remove " + item.toItemStack().toString());
                        }
                    }
                }
            }
